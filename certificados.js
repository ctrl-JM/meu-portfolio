...
window.addEventListener('scroll', () => {
    ...
    if (window.innerWidth <= 768) {
        ...
        // Se não for mobile, garante que o menu está visível
        header.classList.remove('hidden');
    }
});

// Função para trocar idioma - VERSÃO MOBILE (só links de download)
function mudarIdioma(idioma) {
    const urlParams = new URLSearchParams(window.location.search);
    const certificadoId = urlParams.get('id');
    const certificado = certificados[certificadoId];
    ...
    if (certificado && certificado[idioma]) {
        // Em mobile, só atualiza o link de download
        const downloadBtn = document.getElementById(`btn-${idioma}`);
        if (downloadBtn) {
            downloadBtn.href = certificado[idioma];
        }
...
// Função para carregar certificado - VERSÃO DEBUG COMPLETA
function carregarCertificado() {
    console.log('🔍 DEBUG - Função carregarCertificado() executando...');
    
    const urlParams = new URLSearchParams(window.location.search);
    const certificadoId = urlParams.get('id');

    console.log('🔍 DEBUG - Certificado ID:', certificadoId);
    console.log('🔍 DEBUG - URL completa:', window.location.href);
    console.log('🔍 DEBUG - Todos os certificados:', certificados);

    if (certificados[certificadoId]) {
        const certificado = certificados[certificadoId];
        console.log('✅ DEBUG - Certificado encontrado:', certificado);
        
        document.getElementById('certificadoTitulo').textContent = certificado.titulo;
        console.log('✅ DEBUG - Título atualizado para:', certificado.titulo);
        
        mudarIdioma('pt');
    } else {
        console.error('❌ DEBUG - Certificado não encontrado para ID:', certificadoId);
        document.getElementById('certificadoTitulo').textContent = 'Certificado não encontrado';
    }
}

// Função para trocar idioma - VERSÃO DEBUG
function mudarIdioma(idioma) {
    console.log('🔍 DEBUG - mudarIdioma() executando com idioma:', idioma);
    
    const urlParams = new URLSearchParams(window.location.search);
    const certificadoId = urlParams.get('id');
    const certificado = certificados[certificadoId];

    console.log('🔍 DEBUG - Certificado:', certificadoId);
    console.log('🔍 DEBUG - Caminho PDF:', certificado ? certificado[idioma] : 'NÃO ENCONTRADO');

    if (certificado && certificado[idioma]) {
        const iframe = document.getElementById('certificadoFrame');
        console.log('🔍 DEBUG - Iframe encontrado:', iframe);
        
        console.log('📄 DEBUG - Definindo iframe.src para:', certificado[idioma]);
        iframe.src = certificado[idioma];
        console.log('✅ DEBUG - Iframe.src definido como:', iframe.src);
        ...
        console.error('❌ DEBUG - PDF não encontrado para:', certificadoId, 'idioma:', idioma);
    }
}
